...
async function trainModel(inputXs: tf.Tensor2D, outputYs: tf.Tensor2D): Promise<tf.LayersModel> {
...
async function predict(model: tf.LayersModel, pessoa: number[][]): Promise<{ prob: number; index: number }[]> {
    // transformar o array js para o tensor (tfjs)
    const tfInput = tf.tensor2d(pessoa)

    // Faz a predição (output será um vetor de 3 probabilidades)
    const pred = model.predict(tfInput) as tf.Tensor
...
const labelsNomes: string[] = ["premium", "medium", "basic"]; // Ordem dos labels
const tensorLabels: number[][] = [
    [1, 0, 0], // premium - Erick
    [0, 1, 0], // medium - Ana
    [0, 0, 1]  // basic - Carlos
];
...
async function main(): Promise<void> {
    ...
    const pessoaTensorNormalizado: number[][] = [
        [0.33, 1, 0, 0, 1, 0, 0]
    ]
